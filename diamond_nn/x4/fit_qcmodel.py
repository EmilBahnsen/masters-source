...
class OnePassModel(QCModel):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        tf.assert_rank(out, tf.constant((None, 1)))
...
class AllWithXInputModel(tf.keras.Model):
    def __init__(self):
        super(AllWithXInputModel, self).__init__()
        self.input_dense = tf.keras.layers.Dense(17, input_shape=(None, 5))
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        linear_combi = self.input_dense(input_poly_elem)  # Do the init. linear combination + bias with a dense layer

        print(linear_combi[:, 0:3])
        # TODO: Does not work dimentianolly!!!
        !!! FIX !!! THIS CODE IS BACIACALLY THE SAME AS IN x4_*
        U3_1 = U3Layer.matrix_static([
            linear_combi[:, 0:3],
            linear_combi[:, 3:6],
            linear_combi[:, 6:9],
            linear_combi[:, 9:12]
...
with tf.device('cpu'):
    ...
    with open(f'fit_qcmodel_fig/fit_qcmodel_figure_x_c_{t}.pickle', 'bw') as f:
