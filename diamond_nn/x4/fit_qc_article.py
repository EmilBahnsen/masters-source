        ...
        class QC(tf.keras.Model):
            def __init__(self):
                ...
                self.w1 = tf.Variable(init((8,), dtype=float_type),
                                      trainable=True,
                                      dtype=float_type, name='w1')
                self.w2 = tf.Variable(init((8,), dtype=float_type),
                                      trainable=True,
                                      dtype=float_type, name='w2')
            ...
            def call(self, inputs, training=None, mask=None):
                ...
                def apply_qc_to_data(data):
                    ...
                    opers.append(u3_1)
                    # opers.append(iswap_C1T1)
                    # opers.append(iswap_C2T1)
                    # opers.append(iswap_C2T2)
                    # opers.append(iswap_C1T2)
                    opers.append(u_1)
                    # opers.append(u3_2)
...
def main(*args):
    ...
    data_idx = int(args[0])
    # data_idx = 6
    description = f'encoding_U3_U_data{data_idx}'
    # description = 'TEST'
